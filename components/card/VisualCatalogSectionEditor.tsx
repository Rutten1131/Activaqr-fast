...
const LABEL_PRESETS = [
    ...
    { id: 'custom', label: 'Personalizado', singular: '', plural: '', icon: '✏️' },
];
...
export default function VisualCatalogSectionEditor({
    ...
    const [labelSingular, setLabelSingular] = useState<string>(itemLabelSingular !== undefined && itemLabelSingular !== null && itemLabelSingular !== '' ? itemLabelSingular : "Producto");
    const [labelPlural, setLabelPlural] = useState<string>(itemLabelPlural !== undefined && itemLabelPlural !== null && itemLabelPlural !== '' ? itemLabelPlural : "Productos");

    // Sincronizar si cambian los props externos
    useEffect(() => {
        if (itemLabelSingular !== undefined && itemLabelSingular !== null && itemLabelSingular !== '') {
            setLabelSingular(itemLabelSingular);
        }
    }, [itemLabelSingular]);
    useEffect(() => {
        if (itemLabelPlural !== undefined && itemLabelPlural !== null && itemLabelPlural !== '') {
            setLabelPlural(itemLabelPlural);
        }
    }, [itemLabelPlural]);
    ...
    // Emisor central de cambios con soporte para itemLabelSingular e itemLabelPlural
    const emitChange = (updated: {
        ...
        onChange({
            categories: updated.categories !== undefined ? updated.categories : validCategories,
            products: updated.products !== undefined ? updated.products : products,
            categoryImages: updated.categoryImages !== undefined ? updated.categoryImages : categoryImages,
            itemLabelSingular: updated.itemLabelSingular !== undefined ? updated.itemLabelSingular : labelSingular,
            itemLabelPlural: updated.itemLabelPlural !== undefined ? updated.itemLabelPlural : labelPlural,
        });
    };

    const activePreset = useMemo(() => {
        const found = LABEL_PRESETS.find(
            (p) => p.id !== 'custom' && 
                   p.singular.toLowerCase() === (labelSingular || '').trim().toLowerCase() && 
                   p.plural.toLowerCase() === (labelPlural || '').trim().toLowerCase()
        );
    ...
    const handleDeleteCategory = (catName: string, count: number) => {
        ...
        if (count > 0) {
            msg += `\n\nADVERTENCIA: También se eliminarán los ${count} ${count === 1 ? labelSingular.toLowerCase() : labelPlural.toLowerCase()} incluidos en ella.`;
        }
    ...
    const handleAddProduct = () => {
        ...
        const newProd: CatalogProduct = {
            id: `prod_${Date.now()}`,
            name: `Nuevo ${labelSingular}`,
            titulo: `Nuevo ${labelSingular}`,
            price: "$0.00",
            precio: "$0.00",
            description: `Descripción detallada de ${labelSingular.toLowerCase()}.`,
            descripcion: `Descripción detallada de ${labelSingular.toLowerCase()}.`,
    ...
    const handleDeleteProduct = (prodId: string) => {
        if (!confirm(`¿Eliminar este ${labelSingular.toLowerCase()}?`)) return;
        ...
        <div className="w-full space-y-6">
            ...
            {!selectedCategory ? (
                <div className="space-y-4">
                    {/* Selector de Tipo de Catálogo / Etiqueta */}
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                            <div>
                                <span className="text-[11px] font-black uppercase tracking-wider text-navy flex items-center gap-1.5">
                                    🏷️ Tipo de Elementos / Etiqueta del Catálogo
                                </span>
                                <p className="text-[10px] text-gray-500">
                                    Define cómo se llamarán tus ítems en la vCard pública (ej. &ldquo;3 Productos&rdquo;, &ldquo;3 Platos&rdquo;, &ldquo;3 Servicios&rdquo;).
                                </p>
                            </div>
                        </div>
                            ...
                            {LABEL_PRESETS.map((preset) => {
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() => {
                                            if (preset.id !== 'custom') {
                                                setLabelSingular(preset.singular);
                                                setLabelPlural(preset.plural);
                                                emitChange({ itemLabelSingular: preset.singular, itemLabelPlural: preset.plural });
                                            }
                                        }}
                                        className={cn(
                                            "px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all flex items-center gap-1.5 cursor-pointer border",
                                            isSelected
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        emitChange({ itemLabelSingular: val, itemLabelPlural: labelPlural });
                                    }}
                                    placeholder="Singular (ej: Producto)"
                                    className="w-full bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-xs font-bold text-navy outline-none focus:border-orange-500 transition-all"
                                />
                            </div>
                            <div>
                                <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 block mb-1">
                                    Término en Plural (ej: Productos, Platos, Vehículos)
                                </label>
                                <input
                                    type="text"
                                    value={labelPlural}
                                    onChange={(e) => {
                                        const val = e.target.value;
                                        setLabelPlural(val);
                                        emitChange({ itemLabelSingular: labelSingular, itemLabelPlural: val });
                                    }}
                                    placeholder="Plural (ej: Productos)"
                                    className="w-full bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-xs font-bold text-navy outline-none focus:border-orange-500 transition-all"
                                />
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-gray-200">
                        <div>
                            ...
                            <p className="text-[11px] text-gray-500">
                                Toca cualquier categoría para editar sus {labelPlural.toLowerCase()}, fotos, videos y precios.
                            </p>
                        </div>
                        <button
                            type="button"
                                    ...
                                    <div className="relative aspect-[16/9] w-full bg-slate-900 overflow-hidden">
                                        ...
                                        <div className="absolute bottom-2 left-3 z-10 flex items-center gap-1.5">
                                            <span className="px-2.5 py-0.5 rounded-full bg-black/60 backdrop-blur-md border border-white/20 text-[10px] font-black text-white uppercase tracking-wider">
                                                {cat.count} {cat.count === 1 ? (labelSingular || "Producto") : (labelPlural || "Productos")}
                                            </span>
                                        </div>
                                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-gray-50 border border-gray-200 rounded-2xl">
                        ...
                        <div className="flex items-center gap-3">
                            <span className="text-xs font-black uppercase text-orange-600 tracking-wider bg-orange-100 px-3 py-1 rounded-full">
                                📦 {selectedCategory} ({activeCategoryProducts.length} {activeCategoryProducts.length === 1 ? labelSingular : labelPlural})
                            </span>
                            <button
                                type="button"
                                ...
                                style={{ backgroundColor: themeColor }}
                            >
                                <Plus size={16} /> + Añadir {labelSingular}
                            </button>
                        </div>
                    </div>
                    ...
                    {activeCategoryProducts.length === 0 ? (
                        <div className="py-12 border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center text-center p-6 bg-white">
                            ...
                            <h4 className="text-sm font-black text-navy uppercase mb-1">
                                No hay {labelPlural.toLowerCase()} en &ldquo;{selectedCategory}&rdquo;
                            </h4>
                            <p className="text-xs text-gray-500 mb-4 max-w-sm">
                                Añade los {labelPlural.toLowerCase()} que pertenecen a esta categoría con sus fotos, precios y descripción.
                            </p>
                            <button
                                type="button"
                                onClick={handleAddProduct}
                                ...
                                style={{ backgroundColor: themeColor }}
                            >
                                <Plus size={16} className="inline mr-1" /> Añadir Primer {labelSingular}
                            </button>
                        </div>
                    ) : (
                        <div className="space-y-4">
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            title={`Eliminar ${labelSingular.toLowerCase()}`}
                                        >
                                            <Trash2 size={16} />
                                        </button>
